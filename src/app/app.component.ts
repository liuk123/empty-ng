  ...
  constructor(
    ...
    const renderer = {
      heading(text:string, level:number, raw: string, slugger: any) {
        ...
        return `<h${level} id="h_${id}" class="anchor-h">${text}</h${level}>\n`;
      },
      ...
      image(href, title, text){
        ...
        if(index!=-1){
          ...
          if(search){
            ...
            return `
              <img title="${title||text}" alt="${text||title}" class="marked-image" data-source="${href}" data-attrname='src' data-type="attr" width="${ret[0]}px" height="${ret[1]}px">
            `
          }
        }
        return `
          <img title="${title||text}" alt="${text||title}" class="marked-image" data-source="${href}" data-attrname='src' data-type="attr">
        `
      },
      code(code:string, infostring:string,escaped:boolean){
        ...
        if(len>16){
          ...
          + '</code><button class="opt-btn" onClick="((e)=>{e.parentElement.classList.toggle(\'over-hidden\')})(this)"></button></pre>\n';
        }else{
